...
int main(int argc, char *argv[])
{
	...
	uint32_t last_wakeup_ms = acc_integration_get_time();
	...
	while (true)
	{
		...
		if (app_result.result_ready)
		{
			...
			uint32_t next_wake_up_ms = last_wakeup_ms + sleep_time_ms;
			uint32_t current_time_ms = acc_integration_get_time();

			while (next_wake_up_ms > current_time_ms)
			{
				acc_integration_sleep_ms(next_wake_up_ms - current_time_ms);
				current_time_ms = acc_integration_get_time();
			}

			last_wakeup_ms = current_time_ms;
		}
	}
                              ...
                              acc_detector_distance_result_t *detector_result)
{
	...
	do
	{
		...
		if (!acc_detector_distance_process(context->detector_handle, context->buffer, context->detector_cal_result_static,
		                                   &context->detector_cal_result_dynamic,
		                                   &result_available, detector_result))
		{
