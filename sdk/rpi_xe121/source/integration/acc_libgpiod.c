...
static bool gpio_open(int pin, gpio_direction_t direction)
{
	...
	switch (direction)
	{
		case GPIO_DIR_INPUT_INTERRUPT:
			res = gpiod_line_request_rising_edge_events(gpios[pin].line, GPIOD_CONSUMER);
			break;
		case GPIO_DIR_OUTPUT_LOW:
		case GPIO_DIR_OUTPUT_HIGH:
			res = gpiod_line_request_output(gpios[pin].line, GPIOD_CONSUMER, direction == GPIO_DIR_OUTPUT_HIGH ? PIN_HIGH : PIN_LOW);
			break;
		default:
			res = -1;
			assert(false);
...
{
	assert((gpios[pin].direction == GPIO_DIR_OUTPUT_HIGH) || (gpios[pin].direction == GPIO_DIR_OUTPUT_LOW));
	...
	while ((pin_value != PIN_HIGH) && (elapsed_ms < timeout_ms))
	{
		...
		else
		{
			...
			if (event.event_type != GPIOD_LINE_EVENT_RISING_EDGE)
			{
				...
				fprintf(
				    stderr, "Unexpected event_type: %d (expected %d), library version mismatch?\n", event.event_type, GPIOD_LINE_EVENT_RISING_EDGE);
				assert(true);
			}
